...
ast_t parse_leaf(parser_t* p) {
  ...
  if (t.type == TOK_IDENTIFIER)
    return new_ast((node_t){ast_identifier, {.ast_identifier = {t}}});
...
ast_t parse_primary_aux(parser_t* p, int curry) {
  ...
  if (type == TOK_IDENTIFIER || type == TOK_STR_LIT || type == TOK_CHR_LIT ||
      type == TOK_NUM_LIT || type == TOK_WILDCARD) {
    ast_t tree = parse_leaf(p);
    while (is_primary(*p) && curry) {
      ast_t leaf;
      if (parser_peek_type(*p) != TOK_OPEN_PAREN &&
          parser_peek_type(*p) != TOK_OPEN_BRACE)
        leaf = parse_primary_aux(p, 0);
      else
        leaf = parse_expression(p);
      tree = new_ast(
          (node_t){ast_curry, {.ast_curry = {.caller = tree, .arg = leaf}}});
    }
    return tree;
  }
...
ast_t parse_match_case(parser_t* p) {
  ...
  return new_ast(
      (node_t){ast_match_case,
               {.ast_match_case = {.expression = ret, .matcher = match_expr}}});
}
...
ast_t parse_expression(parser_t* p) {
  ...
  if (a == TOK_OPEN_BRACE) {
    ...
    return new_ast((node_t){ast_in, {.ast_in = {binding, expr}}});
  } else if (a == TOK_MATCH) {
...
ast_t parse_type_signature(parser_t* p) {
  ...
  return new_ast((node_t){ast_type, {.ast_type = {.chain = chain}}});
}
...
ast_t parse_let_binding(parser_t* p) {
  ...
  return new_ast((node_t){ast_let_binding,
                          {.ast_let_binding = {.name = name,
                                               .args = args,
                                               .is_void = is_void,
                                               .type_sig = type_sig,
                                               .right = expression}}});
...
ast_t parse_constructor(parser_t* p) {
  ...
  return new_ast((node_t){
      ast_constructor,
      {.ast_constructor = {.name = constructor_name, .signature = type_sig}}});
}
...
ast_t parse_general_type(parser_t* p, int is_rec) {
  ...
  return new_ast((node_t){ast_type_def,
                          {.ast_type_def = {.name = type_name,
                                            .is_rec = is_rec,
