...
__internal_dynamic_array_t __internal_make_array(size_t size) {
  ...
  arr.elem_size = size;
  arr.data = allocate_compiler(arr.elem_size * arr.capacity);
  arr.length = 0;
  return arr;
}
...
int __internal_push_array(__internal_dynamic_array_t* arr, void* elem) {
  ...
  if (arr->length > arr->capacity) {
    arr->capacity *= 2;
    arr->data = reallocate_compiler(arr->data, arr->capacity * arr->elem_size);
  }
...
void* __internal_pop_array(__internal_dynamic_array_t* arr) {
  ...
  void* res = allocate_compiler(arr->elem_size);
  void* src = &arr->data + (--arr->length) * arr->elem_size;
